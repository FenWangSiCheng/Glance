...
actor AIService {
    ...
    private func buildPrompt(from issues: [BacklogIssue], calendarEvents: [CalendarEvent] = []) -> String {
        ...
        if !calendarEvents.isEmpty {
            ...
            以下是今天和未来几天的日历事件（仅供参考，帮助你更好地安排 Backlog 任务的优先级）:
            """
            ...
            在排序时，请考虑日历事件的时间安排，避免在会议密集的时段安排紧急任务。
            """
        }

        return """
        你是一个专业的任务排序助手。请根据以下 Backlog 票据生成一个合理的待办清单。

        要求:
        1. 每个票据就是一个待办项，不需要拆解
        2. 根据优先级、开始日期、截止日期综合考虑，生成合理的执行顺序
        3. 排序规则（按重要性递减）：
           - 优先级高的任务优先
           - 截止日期近的任务优先
           - 开始日期早的任务优先
