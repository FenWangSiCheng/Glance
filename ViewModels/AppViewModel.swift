...
class AppViewModel: ObservableObject {
    ...
    private static func loadTodoItems() -> [TodoItem] {
        ...
        do {
            let storedItems = try JSONSerialization.jsonObject(with: data) as? [[String: Any]] ?? []
            let supportedItems = storedItems.filter { $0["source"] as? String != "calendar" }
            let supportedData = try JSONSerialization.data(withJSONObject: supportedItems)
            return try JSONDecoder().decode([TodoItem].self, from: supportedData)
        } catch {
